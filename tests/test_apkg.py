...
def test_apkg_export_embeds_deck_description(tmp_path: Path) -> None:
    sense = Sense(source_id="hello-1", definition="a greeting", part_of_speech="noun")
    note = DeckNote(
        note_id=stable_note_id(
            provider="fixture",
    ...
    output = tmp_path / "ankglish.apkg"
    blurb = "Sources: wordfreq; Merriam-Webster. Pipeline code: MIT."

    export_apkg([note], output, variant="full", description=blurb)

    with ZipFile(output) as package:
        collection = package.read("collection.anki2")
    assert blurb.encode("utf-8") in collection
